...
rule varscan_somatic:
    ...
    shell:
        "samtools mpileup -C50 -f {input.reference}  "
...
rule vardict_purecn:
    ...
    threads: params['vardict']['threads']
    log:
        "{}/logs/variants/{}-{}.vardict-purecn.log".format(outdir, CANCER_CAPTURE_STR, NORMAL_CAPTURE_STR)
    shell:
        "VarDict -G {input.reference} "
            " -th {threads} "
            "-f {params.min_alt_frac} "
            ...
            " | bcftools view --apply-filters .,PASS " 
            " | vcfsorter.pl {input.reference_dict} /dev/stdin "
            " | bgzip > {params.tmpvcf} 2> {log} && tabix -p vcf {params.tmpvcf} && "
            " bcftools annotate --annotation {input.dbsnp} --columns ID "
            " --output-type z --output {output} {params.tmpvcf} 2>> {log}"
            " && tabix -p vcf {output} && "
            "rm {params.tmpvcf} "
